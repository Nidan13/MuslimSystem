...
@section('title', 'Tambah Kategori Tugas Harian')

@section('content')
<div class="max-w-4xl mx-auto space-y-10 animate-fadeIn">
    <div class="flex justify-between items-end">
        <div>
            <h2 class="text-3xl font-serif font-black text-teal-900 tracking-wide uppercase">Inisialisasi Kategori Tugas</h2>
            <p class="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mt-2 flex items-center gap-2">
                <span class="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_10px_#22d3ee]"></span>
                Pendefinisian Klasifikasi Ritual Baru
            </p>
        </div>
        <a href="{{ route('admin.daily-task-categories.index') }}" class="text-[10px] font-black text-slate-400 hover:text-teal-900 uppercase tracking-widest transition-colors flex items-center gap-2 pb-1">
    ...
    <div class="glass-panel p-10 rounded-[40px] bg-white border-2 border-slate-50 shadow-2xl shadow-slate-200/50">
        <form action="{{ route('admin.daily-task-categories.store') }}" method="POST" class="space-y-8">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required
                        ...
                        placeholder="Misal: Ibadah, Sedekah, Belajar">
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-teal-900/40 uppercase tracking-[0.2em] ml-1">Icon (FontAwesome)</label>
                    <input type="text" name="icon" value="{{ old('icon', 'fas fa-clipboard-check') }}"
                        class="w-full px-6 py-4 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:border-cyan-400 focus:bg-white transition-all outline-none font-bold text-teal-900"
                        placeholder="fas fa-hand-holding-heart">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" rows="4"
                    class="w-full px-6 py-4 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:border-cyan-400 focus:bg-white transition-all outline-none font-bold text-teal-900"
                    placeholder="Jelaskan penggunaan kategori ini... (Opsional)">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-4 p-6 bg-slate-50 rounded-3xl border-2 border-slate-100">
                ...
                <div>
                    ...
                    <span class="text-[9px] text-slate-400 uppercase font-bold">Kategori akan langsung tersedia di Tugas Harian</span>
                </div>
            </div>
