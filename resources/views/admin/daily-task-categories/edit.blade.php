...
@section('title', 'Edit Kategori Tugas Harian')

@section('content')
<div class="max-w-4xl mx-auto space-y-10 animate-fadeIn">
    <div class="flex justify-between items-end">
        <div>
            <h2 class="text-3xl font-serif font-black text-teal-900 tracking-wide uppercase">Modifikasi Kategori Tugas</h2>
            <p class="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mt-2 flex items-center gap-2">
                <span class="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_10px_#22d3ee]"></span>
                Pembaruan Parameter Klasifikasi
            </p>
        </div>
    ...
    <div class="glass-panel p-10 rounded-[40px] bg-white border-2 border-slate-50 shadow-2xl shadow-slate-200/50">
        <form action="{{ route('admin.daily-task-categories.update', $category) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
