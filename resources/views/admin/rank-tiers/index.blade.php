...
@section('title', 'Registri Tingkat Otoritas')

@section('content')
<div class="space-y-10">
    <div class="flex justify-between items-end">
        <div>
            <h1 class="text-4xl font-serif font-black text-teal-900 tracking-wide uppercase">Tingkat Otoritas</h1>
            <p class="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mt-2 flex items-center gap-2">
                <span class="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.5)]"></span>
                Mendefinisikan Struktur Rank Global & Penskalaan Kekuatan
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        ...
        <div class="glass-panel p-1 rounded-[40px] relative overflow-hidden group hover:-translate-y-2 transition-all duration-500 border-2 {{ $tier->name == 'S' ? 'border-gold-400/30' : 'border-slate-100' }}">
            ...
            <div class="p-8 pb-10">
                ...
                <div class="flex flex-col items-center mb-10">
                    <div class="text-7xl font-serif font-black italic tracking-tighter drop-shadow-sm group-hover:scale-110 transition-transform duration-700" style="{{ $tier->color_code ? 'color: ' . $tier->color_code : 'color: #134e4a' }}">
                        {{ $tier->slug }}
                    </div>
                ...
                <div class="space-y-4 px-2">
                    <div class="flex justify-between items-center py-3 border-y-2 border-slate-50">
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Min. Syarat</span>
                    ...
                    <div class="flex justify-between items-center py-1">
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Hunter Aktif</span>
        ...
        <a href="{{ route('admin.rank-tiers.create') }}" class="group p-1 rounded-[40px] border-2 border-dashed border-slate-200 hover:border-cyan-400 hover:bg-cyan-50/30 transition-all duration-500 flex items-center justify-center min-h-[300px]">
             <div class="flex flex-col items-center text-slate-300 group-hover:text-cyan-500 transition-colors">
                 ...
                 <span class="text-[10px] font-black uppercase tracking-[0.4em]">Inisialisasi Tingkat Baru</span>
             </div>
        </a>
    </div>
</div>
