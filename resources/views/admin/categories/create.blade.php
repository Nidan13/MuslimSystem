...
<div class="max-w-4xl mx-auto space-y-10 animate-fadeIn">
    ...
    <div class="flex justify-between items-end">
        <div>
            <h2 class="text-3xl font-serif font-black text-teal-900 tracking-wide uppercase">Inisialisasi Kategori</h2>
            <p class="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mt-2 flex items-center gap-2">
                <span class="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_10px_#22d3ee]"></span>
                Pendefinisian Klasifikasi Baru
            </p>
        </div>
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required
                        ...
                        placeholder="Misal: Ibadah Harian">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex gap-3">
                        <input type="color" name="color" value="{{ old('color', '#0f4c5c') }}"
                            class="w-16 h-14 p-1 rounded-xl bg-slate-50 border-2 border-slate-100 cursor-pointer">
                        <input type="text" id="color_text" value="{{ old('color', '#0f4c5c') }}" readonly
            ...
            <div id="metadata-fields" class="grid grid-cols-1 md:grid-cols-2 gap-8 hidden">
                ...
                <div id="rank-fields" class="space-y-2 hidden">
                    ...
                    <input type="number" name="metadata[min_level]" value="{{ old('metadata.min_level', 1) }}"
                        class="w-full px-6 py-4 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:border-cyan-400 focus:bg-white transition-all outline-none font-bold text-teal-900"
                        placeholder="1-99">
                </div>
                ...
                <div id="dungeon-fields" class="space-y-2 hidden">
                    ...
                    <input type="number" name="metadata[max_participants]" value="{{ old('metadata.max_participants', 1) }}"
                        class="w-full px-6 py-4 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:border-cyan-400 focus:bg-white transition-all outline-none font-bold text-teal-900"
            ...
            <div class="flex items-center gap-4 p-6 bg-slate-50 rounded-3xl border-2 border-slate-100">
                ...
                <div>
                    ...
                    <span class="text-[9px] text-slate-400 uppercase font-bold">Kategori akan langsung tersedia di sistem</span>
                </div>
            </div>
