...
@section('title', 'Edit Misi: ' . $quest->title)
...
<div class="w-full animate-fadeIn">
    ...
    <div class="flex items-center gap-6 mb-10">
        ...
        <div>
            <h1 class="text-3xl font-serif font-black text-teal-900 tracking-tight uppercase italic leading-none">Update <span class="text-cyan-500 font-sans tracking-normal not-italic ml-1">Misi</span></h1>
            <p class="text-slate-400 text-[10px] font-bold uppercase tracking-[0.3em] mt-1">ID Misi: #{{ str_pad($quest->id, 5, '0', STR_PAD_LEFT) }}</p>
        </div>
    </div>
    ...
    <div class="glass-panel p-10 rounded-[40px] bg-white border-2 border-slate-50 shadow-xl relative overflow-hidden">
        ...
        <form action="{{ route('admin.quests.update', $quest) }}" method="POST" class="space-y-10 relative z-10">
            @csrf @method('PUT')
...
=======
                            <select name="rank_tier_id" 
                                ...
                                <option value="{{ $rank->id }}" {{ $quest->rank_tier_id == $rank->id ? 'selected' : '' }}>TIER {{ $rank->slug }} - {{ $rank->name }}</option>
>>>>>>> main
                                @endforeach
                            </select>
                            <i class="fas fa-chevron-down absolute right-8 top-1/2 -translate-y-1/2 text-cyan-500 pointer-events-none group-hover:rotate-180 transition-transform"></i>
                ...
                <div class="space-y-3">
                    ...
                    <textarea name="description" rows="4" 
                        ...
                        placeholder="Detail instruksi hunt...">{{ old('description', $quest->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="p-8 bg-slate-50 rounded-[32px] border border-slate-100 group hover:border-blue-300 transition-colors">
                        ...
                        <input type="number" name="reward_exp" value="{{ old('reward_exp', $quest->reward_exp) }}" 
                    ...
                    <div class="p-8 bg-slate-50 rounded-[32px] border border-slate-100 group hover:border-red-300 transition-colors">
                        ...
                        <input type="number" name="penalty_fatigue" value="{{ old('penalty_fatigue', $quest->penalty_fatigue) }}" 
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex bg-white rounded-lg p-1 border border-slate-200">
                            <label class="flex items-center gap-2 px-3 py-1 cursor-pointer rounded-md transition-all has-[:checked]:bg-teal-900 has-[:checked]:text-white">
                                <input type="radio" name="schedule_type" value="specific" {{ !$isRecurring ? 'checked' : '' }} onchange="toggleScheduleFields()" class="hidden">
                                <span class="text-[9px] font-black uppercase tracking-widest">Spesifik</span>
                            </label>
                            <label class="flex items-center gap-2 px-3 py-1 cursor-pointer rounded-md transition-all has-[:checked]:bg-teal-900 has-[:checked]:text-white">
                                <input type="radio" name="schedule_type" value="recurring" {{ $isRecurring ? 'checked' : '' }} onchange="toggleScheduleFields()" class="hidden">
                                <span class="text-[9px] font-black uppercase tracking-widest">Rutin</span>
                            </label>
                        </div>
                    </div>

                    <div id="specific-fields" class="grid grid-cols-2 gap-6 {{ $isRecurring ? 'hidden' : '' }} animate-fadeIn">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Tgl Mulai</label>
                            <input type="datetime-local" name="starts_at" value="{{ $quest->starts_at?->format('Y-m-d\TH:i') }}" class="w-full bg-white border border-slate-100 rounded-xl p-4 text-xs font-black text-teal-900 focus:border-cyan-400 focus:outline-none font-mono">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Tgl Berakhir</label>
                            <input type="datetime-local" name="expires_at" value="{{ $quest->expires_at?->format('Y-m-d\TH:i') }}" class="w-full bg-white border border-slate-100 rounded-xl p-4 text-xs font-black text-red-600 focus:border-red-400 focus:outline-none font-mono">
                        </div>
                    </div>

                    <div id="recurring-fields" class="grid grid-cols-2 gap-6 {{ !$isRecurring ? 'hidden' : '' }} animate-fadeIn">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Jam Aktif</label>
                            <input type="time" name="start_time" value="{{ $quest->start_time?->format('H:i') }}" class="w-full bg-white border border-slate-100 rounded-xl p-4 text-lg font-black text-teal-900 focus:border-cyan-400 focus:outline-none font-mono">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Jam Selesai</label>
                            <input type="time" name="end_time" value="{{ $quest->end_time?->format('H:i') }}" class="w-full bg-white border border-slate-100 rounded-xl p-4 text-lg font-black text-red-600 focus:border-red-400 focus:outline-none font-mono">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div id="requirements-container" class="space-y-4">
                        ...
                        <div class="flex gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 animate-fadeIn items-center group">
                            <input type="text" name="req_keys[]" value="{{ $key }}" placeholder="ID_TARGET" class="flex-1 bg-white border border-slate-200 rounded-xl px-5 py-3 text-[10px] font-black uppercase tracking-widest focus:border-teal-900 focus:outline-none shadow-inner">
                            <input type="number" name="req_values[]" value="{{ $value }}" placeholder="VAL" class="w-24 bg-white border border-slate-200 rounded-xl px-4 py-3 text-center font-black focus:border-teal-900 focus:outline-none shadow-inner">
                        ...
                        @empty
                        <div class="py-6 text-center text-slate-200 text-[10px] font-black uppercase tracking-widest italic border-2 border-dashed border-slate-100 rounded-3xl">
                            Belum ada parameter khusus
                        </div>
                        @endforelse
                    </div>
                </div>
...
<script>
    ...
    function addRequirement() {
        ...
        row.innerHTML = `
            <input type="text" name="req_keys[]" placeholder="ID_TARGET" class="flex-1 bg-white border border-slate-200 rounded-xl px-5 py-3 text-[10px] font-black uppercase tracking-widest focus:border-teal-900 focus:outline-none shadow-inner">
