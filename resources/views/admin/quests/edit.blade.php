...
@section('title', 'Rekalibrasi Mandat: ' . $quest->title)
...
<div class="max-w-5xl mx-auto animate-fadeIn">
    ...
    <div class="flex items-center gap-8 mb-12">
        ...
        <div>
            <h1 class="text-5xl font-serif font-black text-teal-900 tracking-tight uppercase">Rekalibrasi Mandat</h1>
            <p class="text-slate-500 text-[11px] font-bold uppercase tracking-[0.4em] mt-3 flex items-center gap-2">
                <span class="w-2.5 h-2.5 rounded-full bg-cyan-400 animate-pulse shadow-[0_0_12px_rgba(34,211,238,0.5)]"></span>
    ...
    <div class="glass-panel p-12 rounded-[56px] bg-white/90 backdrop-blur-xl border-2 border-slate-50 shadow-3xl relative overflow-hidden">
        <div class="absolute -right-32 -top-32 w-96 h-96 bg-cyan-400/10 rounded-full blur-[120px] pointer-events-none opacity-50"></div>
        ...
        <form action="{{ route('admin.quests.update', $quest) }}" method="POST" class="space-y-12 relative z-10">
            ...
            <div class="space-y-10">
                ...
                <div class="space-y-4">
                    ...
                    <textarea name="description" rows="5" 
                        ...
                        placeholder="MASUKKAN DETAIL MANDAT SISTEM...">{{ old('description', $quest->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div class="p-8 bg-slate-50 rounded-[32px] border-2 border-slate-100 shadow-inner group hover:border-gold-300 transition-colors">
                        ...
                        <input type="number" name="reward_soul_points" value="{{ old('reward_soul_points', $quest->reward_soul_points) }}" 
                ...
                <div class="p-10 bg-slate-50 rounded-[48px] border-2 border-slate-100 shadow-inner space-y-10">
                    <div class="flex justify-between items-center px-4">
                        ...
                        <div class="flex gap-10">
                            <label class="flex items-center gap-4 cursor-pointer group">
                                <input type="radio" name="schedule_type" value="specific" {{ !$isRecurring ? 'checked' : '' }} onchange="toggleScheduleFields()" class="w-6 h-6 text-teal-900 focus:ring-cyan-400 border-slate-200">
                            ...
                            <label class="flex items-center gap-4 cursor-pointer group">
                                <input type="radio" name="schedule_type" value="recurring" {{ $isRecurring ? 'checked' : '' }} onchange="toggleScheduleFields()" class="w-6 h-6 text-teal-900 focus:ring-cyan-400 border-slate-200">
                    ...
                    <div id="specific-fields" class="grid grid-cols-2 gap-10 {{ $isRecurring ? 'hidden' : '' }} animate-fadeIn">
                        <div class="space-y-4">
                            ...
                            <input type="datetime-local" name="starts_at" value="{{ $quest->starts_at?->format('Y-m-d\TH:i') }}" class="w-full bg-white border-2 border-slate-100 rounded-[20px] p-5 text-sm font-black text-teal-900 focus:border-cyan-400 focus:outline-none shadow-sm transition-all font-mono uppercase">
                        </div>
                        <div class="space-y-4">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Terminasi Node</label>
                            <input type="datetime-local" name="expires_at" value="{{ $quest->expires_at?->format('Y-m-d\TH:i') }}" class="w-full bg-white border-2 border-slate-100 rounded-[20px] p-5 text-sm font-black text-red-600 focus:border-red-400 focus:outline-none shadow-sm transition-all font-mono uppercase">
                        </div>
                    </div>
                    ...
                    <div id="recurring-fields" class="grid grid-cols-2 gap-10 {{ !$isRecurring ? 'hidden' : '' }} animate-fadeIn">
                        <div class="space-y-4">
                            ...
                            <input type="time" name="start_time" value="{{ $quest->start_time?->format('H:i') }}" class="w-full bg-white border-2 border-slate-100 rounded-[20px] p-5 text-lg font-black text-teal-900 focus:border-cyan-400 focus:outline-none shadow-sm transition-all font-mono">
                        </div>
                        <div class="space-y-4">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Deaktivasi Harian</label>
                            <input type="time" name="end_time" value="{{ $quest->end_time?->format('H:i') }}" class="w-full bg-white border-2 border-slate-100 rounded-[20px] p-5 text-lg font-black text-red-600 focus:border-red-400 focus:outline-none shadow-sm transition-all font-mono">
                        </div>
                    </div>
                </div>
                    ...
                    <div id="requirements-container" class="space-y-6">
                        ...
                        <div class="grid grid-cols-12 gap-6 items-center animate-fadeIn p-6 bg-slate-50 rounded-[32px] border-2 border-white shadow-sm group">
                             <div class="col-span-8">
                                <input type="text" name="req_keys[]" value="{{ $key }}" placeholder="misal: ID_TARGET" class="w-full bg-white border-2 border-slate-100 rounded-2xl px-8 py-5 font-black uppercase tracking-widest text-[11px] focus:border-teal-900 focus:outline-none shadow-inner transition-all">
                             </div>
                             <div class="col-span-3">
                                <input type="number" name="req_values[]" value="{{ $value }}" placeholder="VAL" class="w-full bg-white border-2 border-slate-100 rounded-2xl px-6 py-5 font-black text-center focus:border-teal-900 focus:outline-none shadow-inner transition-all font-mono text-lg">
                             </div>
            ...
            <!-- Submit Section -->
            <div class="pt-12 border-t-2 border-slate-50 text-center">
                <button type="submit" class="w-full group relative overflow-hidden bg-teal-900 hover:bg-teal-800 py-8 rounded-[32px] font-serif font-black text-white uppercase tracking-[0.5em] shadow-3xl shadow-teal-950/40 transition-all active:scale-[0.98] border-t border-white/10">
                    ...
                    <span class="relative flex items-center justify-center gap-6 text-lg">
                        SINKRONISASI MATRIKS MANDAT
                        <i class="fas fa-sync text-cyan-400 transition-all group-hover:rotate-180 text-xl"></i>
                    </span>
                </button>
            </div>
        </form>
