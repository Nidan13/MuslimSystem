...
@section('title', 'Recalibrate Progres Matrix')
...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center gap-6 mb-10">
        ...
        <div>
            <h1 class="text-4xl font-serif font-black text-teal-900 tracking-wide uppercase">Recalibrate Step</h1>
            <p class="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mt-2 flex items-center gap-2">
                <span class="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.5)]"></span>
                Target: Level {{ $config->level }} Parameters
            </p>
        </div>
    </div>
    ...
    <div class="glass-panel p-12 rounded-[40px] relative overflow-hidden">
        ...
        <form action="{{ route('admin.level-configs.update', $config) }}" method="POST" class="space-y-10 relative z-10">
            @csrf @method('PUT')
            ...
            <div class="space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div>
                        ...
                        <input type="number" name="level" value="{{ $config->level }}" required 
                            class="w-full bg-slate-50 border-2 border-slate-200 rounded-[24px] text-teal-900 p-6 focus:border-cyan-400 focus:bg-white outline-none transition-all font-mono font-black text-xl shadow-inner cursor-not-allowed" readonly>
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-gold-500 uppercase mb-3 tracking-[0.3em] ml-1">Stat points yield (AP)</label>
                        <input type="number" name="stat_points_reward" value="{{ $config->stat_points_reward }}" required 
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="xp_required" value="{{ $config->xp_required }}" required 
