...
<div class="max-w-5xl animate-fadeIn">
    <div class="flex items-center gap-6 mb-12">
        ...
        <div>
            <h2 class="text-3xl font-serif font-black text-teal-900 tracking-tight uppercase">Tambah Section</h2>
            <p class="text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em] mt-2">Inisialisasi Komponen Antarmuka Utama</p>
        </div>
    </div>

    <form action="{{ route('admin.landing-page.sections.store') }}" method="POST" enctype="multipart/form-data" class="glass-panel p-12 rounded-[40px] bg-white border-2 border-slate-50 shadow-xl shadow-slate-200/50 space-y-10">
        @csrf
        
        <div class="grid md:grid-cols-2 gap-10">
            <div class="space-y-4">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Judul Utama (Title)</label>
                <input type="text" name="title" value="{{ old('title') }}" placeholder="Contoh: Fitur Unggulan" class="w-full px-8 py-5 rounded-2xl bg-slate-50 border-2 border-slate-50 focus:border-cyan-400 focus:bg-white transition-all outline-none font-serif text-teal-900 shadow-inner" required>
            </div>

            <div class="space-y-4">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Sub-judul (Subtitle)</label>
                <input type="text" name="subtitle" value="{{ old('subtitle') }}" placeholder="Contoh: Kembangkan Potensi Spiritual" class="w-full px-8 py-5 rounded-2xl bg-slate-50 border-2 border-slate-50 focus:border-cyan-400 focus:bg-white transition-all outline-none font-serif text-teal-900 shadow-inner">
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-10">
            <div class="space-y-4">
                ...
                <select name="type" class="w-full px-8 py-5 rounded-2xl bg-slate-50 border-2 border-slate-50 focus:border-cyan-400 focus:bg-white transition-all outline-none font-serif text-teal-900 shadow-inner appearance-none cursor-pointer" required>
                    <option value="hero">Hero Section (Bisa Slider/Tunggal)</option>
                    <option value="feature_cards">Evolution Cards (3 Kolom Grid)</option>
                    <option value="human_centric_grid">Feature Grid (4 Kolom Mini)</option>
                    <option value="about">About / Text with Image</option>
            ...
            <div class="space-y-4">
                ...
                <select name="style" class="w-full px-8 py-5 rounded-2xl bg-slate-50 border-2 border-slate-50 focus:border-cyan-400 focus:bg-white transition-all outline-none font-serif text-teal-900 shadow-inner appearance-none cursor-pointer" required>
                    <option value="default">Default (Kiri ke Kanan)</option>
                    <option value="reversed">Terbalik (Kanan ke Kiri)</option>
                    <option value="dark">Tema Gelap (Dark Mode)</option>
                    <option value="cards">Format Kartu (Cards)</option>
                </select>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-10">
            <div class="space-y-4">
                ...
                <input type="number" name="order" value="{{ old('order', 1) }}" class="w-full px-8 py-5 rounded-2xl bg-slate-50 border-2 border-slate-50 focus:border-cyan-400 focus:bg-white transition-all outline-none font-serif text-teal-900 shadow-inner" required>
            </div>
        ...
        <div class="space-y-4">
            ...
            <textarea name="content" rows="6" placeholder="Masukkan konten narasi..." class="w-full px-8 py-5 rounded-[2.5rem] bg-slate-50 border-2 border-slate-50 focus:border-cyan-400 focus:bg-white transition-all outline-none font-serif text-teal-900 shadow-inner">{{ old('content') }}</textarea>
        </div>

        <div class="grid md:grid-cols-2 gap-10">
            <div class="space-y-4">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Teks Tombol Aksi (Opsional)</label>
                <input type="text" name="button_text" value="{{ old('button_text') }}" placeholder="Contoh: Gabung Sekarang" class="w-full px-8 py-5 rounded-2xl bg-slate-50 border-2 border-slate-50 focus:border-cyan-400 focus:bg-white transition-all outline-none font-serif text-teal-900 shadow-inner">
            </div>

            <div class="space-y-4">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Link Tombol Aksi (Opsional)</label>
                <input type="text" name="button_url" value="{{ old('button_url') }}" placeholder="Contoh: /register atau https://..." class="w-full px-8 py-5 rounded-2xl bg-slate-50 border-2 border-slate-50 focus:border-cyan-400 focus:bg-white transition-all outline-none font-serif text-teal-900 shadow-inner">
            </div>
        </div>
        ...
        <div class="pt-10 border-t border-slate-100 space-y-6">
            ...
            <div id="items-container" class="space-y-4">
                <!-- Repeater item rows will be appended here by JS -->
            </div>
        </div>
