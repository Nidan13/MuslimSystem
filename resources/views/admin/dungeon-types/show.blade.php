...
@section('title', 'Detail Kategori: ' . $dungeonType->name)
...
<div class="w-full space-y-10">
        ...
        <div class="flex items-center gap-6">
            ...
            <div>
                ...
                <p class="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mt-2 flex items-center gap-2">
                    ...
                    Kategori Gerbang <span class="text-slate-200">|</span> ID: TYPE-{{ str_pad($dungeonType->id, 3, '0', STR_PAD_LEFT) }}
                </p>
            </div>
        </div>
        <div class="flex gap-4">
            <a href="{{ route('admin.dungeon-types.edit', $dungeonType) }}" class="px-8 py-4 bg-teal-900 text-white rounded-2xl font-serif font-black uppercase tracking-widest text-[10px] shadow-xl shadow-teal-950/20 hover:bg-teal-800 transition-all group active:scale-95">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div class="glass-panel p-12 rounded-[50px] bg-teal-900 border-2 border-white/5 relative overflow-hidden group flex flex-col items-center justify-center text-center">
            ...
            <h3 class="text-[9px] font-black text-cyan-400/60 uppercase tracking-[0.5em] mb-10">Batas Peserta Maksimal</h3>
            ...
            <p class="text-[10px] font-black text-white/40 uppercase tracking-[0.3em]">Hunter Dalam Party</p>
        </div>
        ...
        <div class="lg:col-span-2 glass-panel overflow-hidden rounded-[50px] bg-white border-2 border-slate-50 shadow-xl">
            <div class="p-10 border-b-2 border-slate-50 flex justify-between items-center">
                <h3 class="text-[10px] font-black text-teal-900/40 uppercase tracking-[0.5em]">Daftar Gerbang Terkait</h3>
                <span class="px-4 py-1.5 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black text-teal-900 tracking-widest">{{ $dungeonType->dungeons->count() }} GERBANG AKTIF</span>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-slate-50 uppercase">
                            <th class="px-10 py-6 text-[9px] font-black text-teal-900/40 tracking-widest">Nama Gerbang</th>
                            <th class="px-10 py-6 text-[9px] font-black text-teal-900/40 tracking-widest text-right">Status</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y-2 divide-slate-50">
                        ...
                        <tr>
                            <td colspan="2" class="px-10 py-20 text-center opacity-30 italic font-medium uppercase tracking-[0.2em] text-teal-900">
                                Belum ada gerbang terkait pada kategori ini
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
