...
<div class="space-y-10">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 animate-fadeIn">
        <div>
            <h2 class="text-4xl font-serif font-black text-teal-900 tracking-wide uppercase">Kategori Kajian</h2>
            <p class="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mt-2 flex items-center gap-2">
                <span class="w-2 h-2 rounded-full bg-cyan-400 animate-pulse shadow-[0_0_10px_#22d3ee]"></span>
                Master Data Kategori Kajian (Video Islami)
            </p>
        </div>
        
        <div class="flex flex-col md:flex-row items-center gap-4">
            <!-- Add Button -->
            <a href="{{ route('admin.islamic-video-categories.create') }}" class="group relative px-8 py-4 rounded-2xl bg-teal-900 text-white shadow-xl shadow-teal-950/20 hover:bg-teal-800 transition-all active:scale-95 overflow-hidden font-serif uppercase tracking-widest text-[10px] font-black">
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50 font-sans">
                    ...
                    <tr class="group hover:bg-slate-50/50 transition-colors">
                        <td class="py-6 px-4">
                            <span class="text-[10px] font-black text-slate-300 font-mono tracking-tighter uppercase whitespace-nowrap">#KJN-{{ str_pad($category->id, 3, '0', STR_PAD_LEFT) }}</span>
                        </td>
                        <td class="py-6 px-6">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg flex items-center justify-center border-2 border-slate-100 shadow-sm" style="background-color: {{ $category->color ?? '#f1f5f9' }}">
                                    <i class="{{ $category->icon ?? 'fas fa-book-open' }} text-[10px] {{ $category->color ? 'text-white' : 'text-slate-400' }}"></i>
                                </div>
                        ...
                        <td class="py-6 px-6 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-3 transition-all duration-300">
                                <a href="{{ route('admin.islamic-video-categories.edit', $category) }}" class="p-3 bg-white border-2 border-slate-100 text-teal-900 hover:text-cyan-500 hover:border-cyan-200 rounded-xl transition-all shadow-sm active:scale-95 leading-none shadow-slate-200/50" title="Edit Kategori">
                                    <i class="fas fa-sliders text-xs"></i>
                                </a>
                                <form action="{{ route('admin.islamic-video-categories.destroy', $category) }}" method="POST" class="inline" onsubmit="return confirm('Hapus kategori ini?')">
                                    @csrf @method('DELETE')
