...
@section('title', 'Modifikasi Kategori Quest')
...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center gap-6 mb-10">
        ...
        <div>
            <h1 class="text-4xl font-serif font-black text-teal-900 tracking-wide uppercase">Modifikasi Taksonomi</h1>
            <p class="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em]">Memperbarui node klasifikasi: {{ $questType->name }}</p>
        </div>
    </div>
    ...
    <div class="glass-panel p-12 rounded-[40px] relative overflow-hidden">
        ...
        <form action="{{ route('admin.quest-types.update', $questType) }}" method="POST" class="space-y-10 relative z-10">
            @csrf @method('PUT')
            
            <div class="grid grid-cols-2 gap-10">
                <div class="col-span-1">
                    <label class="block text-[10px] font-black text-teal-900/40 uppercase mb-3 tracking-[0.3em] ml-1">Nama Kategori</label>
                    <input type="text" name="name" value="{{ $questType->name }}" required 
                ...
                <div class="col-span-1">
                    <label class="block text-[10px] font-black text-teal-900/40 uppercase mb-3 tracking-[0.3em] ml-1">Pengenal Logika (Slug)</label>
                    <input type="text" name="slug" value="{{ $questType->slug }}" required 
                ...
                <div class="col-span-2">
                    <label class="block text-[10px] font-black text-teal-900/40 uppercase mb-3 tracking-[0.3em] ml-1">Parameter Deskriptif</label>
                    <textarea name="description" rows="5" 
                        class="w-full bg-slate-50 border-2 border-slate-200 rounded-[32px] text-slate-700 p-8 outline-none focus:border-cyan-400 focus:bg-white transition-all font-medium placeholder-slate-200 shadow-inner">{{ $questType->description }}</textarea>
                </div>
            </div>
