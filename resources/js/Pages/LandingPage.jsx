...
const HeroDownloadButton = ({ downloadUrl }) => {
    ...
    const handleDownload = () => {
        ...
        setTimeout(() => {
            // Ensure downloadUrl is valid before attempting download
            if (downloadUrl) {
            ...
            {items.map((item, i) => {
                ...
                return (
                    <div key={i} className={`absolute inset-0 transition-all duration-1000 ${active === i ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12 pointer-events-none'}`}>
                        {item.image_url ? (
                            <img src={item.image_url} alt={item.title || 'Slider Image'} className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-1000" />
                        ) : (
                ...
                <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center relative z-10">
                    <RevealSection className={`${isReversed ? 'lg:order-2' : 'lg:order-1'}`} direction={isReversed ? "right" : "left"}>
                        ...
                        <h1 className={`text-5xl lg:text-8xl font-serif font-black leading-[0.9] mb-8 uppercase tracking-tighter ${isDark ? 'text-white' : 'text-white'}`}>
                            {(section.title || "").split(' ').map((word, i, array) => (
                                <span key={i} className={i === 0 ? 'block' : i === array.length - 1 ? 'block text-nu-teal-light drop-shadow-sm' : ''}>
                                    {word}{' '}
                                </span>
                            ))}
                        </h1>
                            ...
                            {latestNews.map((news, i) => (
                                <RevealSection key={i} delay={`${i * 200}ms`} direction="up">
                                    <div className="group bg-white rounded-[3rem] border border-slate-100 overflow-hidden hover:shadow-[0_40px_80px_-15px_rgba(10,47,76,0.1)] transition-all duration-700 hover:-translate-y-4">
                                        ...
                                        <div className="p-10">
                                            <p className="text-[10px] font-bold text-nu-teal uppercase tracking-[0.3em] mb-4">
                                                {(() => {
                                                    const dateVal = news.published_at || news.created_at;
                                                    return dateVal ? new Date(dateVal).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : '-';
                                                })()}
                                            </p>
