...
export default function MainLayout({ children }) {
    const { auth, downloadUrl, theme = {} } = usePage().props;
    ...
    const hexToRgba = (hex, alpha = 1) => {
        if (!hex || !/^#([A-Fa-f0-9]{3}){1,2}$/.test(hex)) return `rgba(0,139,118,${alpha})`;
        let r=0, g=0, b=0;
        if (hex.length == 4) {
    ...
    const primaryColor = theme?.primary || '#008b76';
    const navbarColor = theme?.navbar || '#008b76';
    const footerColor = theme?.footer || '#0a2f4c';
